...
function Inspector(schema, alias) {
    ...
    this._alias2search = {}; // alias-search_field object
...
Inspector.prototype.process = function(schema, parent, ptype) {
    ...
    _.each(schema, function(value, key) {
        ...
        // search index
        var search = dot.get(curr, 'settings.search');
        ...
        if(search) {
            ...
            // text fields
            if(search.type && search.type.substr(0, 5) == 'text_')
                _save.search.field = curr.alias+'_'+search.type;
            ...
            if(fieldSlug) {
                if(search.store && arr)
                    _save.search.field = curr.alias+'_'+fieldSlug+'ms';
                else if(search.store)
                    _save.search.field = curr.alias+'_'+fieldSlug+'s';
                else if(arr)
                    _save.search.field = curr.alias+'_'+fieldSlug+'m';
                else
                    _save.search.field = curr.alias+'_'+fieldSlug;
            }

            // set schema as searchable
            if(_save.search.field) {
                self._searchable = true;
                self._alias2search[curr.alias] = _save.search.field;
            }
        }
    });
};
...
Inspector.prototype.init = function() {
    ...
    var Schema = {
        ...
        SearchFields : self._alias2search
    };
