...
FeedParser.prototype.saveContent = function(appkey, itemkey, itemurl, item, channel, terms, content) {
    ...
    new this._schema('feed.contents').init(this._app).post(body, function(err, content) {
        ...
        if(self._app.boot.shortener) {
            var shortURLPromise = self._app.boot.shortener.generate({
                URL: itemurl
            });

            shortURLPromise.then(function(mongodbDoc) {
                if(mongodbDoc && mongodbDoc.hash)
                    new self._schema('feed.contents').init(self._app).put(content._id.toString(), {shortener: mongodbDoc.hash}, function(err, raw) {});
            }, function(error) {
                if(error)
                    self._log.error(group, error);
            });
