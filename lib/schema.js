...
Schema.prototype.init = function () {
    ...
    this._methodMaster = this._req ? this._req.methodMaster : undefined;
    ...
    // is api call?
    this._api = (this._res && this._res.apiResponse);

    // is user?
    this._user = (this._req && this._req.user) ? this._req.user.id : false;
...
Schema.prototype.getFunc = function (obj, self, cb) {
    ...
    if( ! self._methodMaster && self._owner && dot.get(self._owner, 'protect.get') && self._res && self._res.apiResponse)
        obj[self._owner.alias] = self._req.user.id;
...
Schema.prototype.getById = function (id, cb) {
    ...
    // set acl owner protection (api response ise owner protection uygula)
    if( ! this._methodMaster && this._owner && dot.get(this._owner, 'protect.getid') && this._res && this._res.apiResponse)
        cond[this._owner.field] = this._req.user.id;
...
Schema.prototype.post = function (obj, cb) {
    ...
    // set acl owner protection (api response ise owner protection uygula)
    if(this._owner && dot.get(this._owner, 'protect.post') && this._res && this._res.apiResponse) {
        if( ! this._methodMaster )
            obj[this._owner.alias] = this._req.user.id;
        else if( ! obj[this._owner.alias] || ! obj[this._owner.field] )
            obj[this._owner.alias] = this._req.user.id;

        // check profile id
        if(this._owner.profile && this._req.user.profile) {
            if( ! this._methodMaster )
                obj[this._owner.profile.alias] = this._req.user.profile;
...
Schema.prototype.put = function (id, obj, cb) {
    ...
    if(this._req && this._req.system_AppId) {
        var appId = this._req.system_AppId;
    ...
    // set acl owner protection (api response ise owner protection uygula)
    if( ! this._methodMaster && this._owner && dot.get(this._owner, 'protect.put') && this._res && this._res.apiResponse) {
        if(where)
            where[this._owner.alias] = this._req.user.id;
        else
            cond[this._owner.alias] = this._req.user.id;
    }
    ...
    async.waterfall(a, function(err, parsed, doc) {
        ...
        // set acl owner protection (api response ise owner protection uygula)
        if( ! self._methodMaster && self._owner && dot.get(self._owner, 'protect.put') && self._res && self._res.apiResponse) {
...
Schema.prototype.remove = function (id, cb) {
    ...
    // set acl owner protection (api response ise owner protection uygula)
    if( ! this._methodMaster && this._owner && dot.get(this._owner, 'protect.remove') && this._res.apiResponse)
        cond[this._owner.field] = this._req.user.id;
