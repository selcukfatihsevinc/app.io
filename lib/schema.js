...
Schema.prototype.init = function () {
    ...
    // set redis client for query (caching vs.)
    this._redis = this._app.core.redis ? this._app.core.redis.a : false;
    this._config.query.redis = this._redis;
    ...
    this._validate = {
        empty: function (schema, candidate, cb) {
            ...
            if(typeof candidate == 'undefined' || self.type(candidate) == '[object Date]')
                return cb();

            if(php.empty(candidate))
...
Schema.prototype.errors = function (err) {
    ...
    switch(err.name) {
        case 'ValidationError':
            ...
            return this._cb ? this._cb(resp) : this._next(this._http.UnprocessableEntity(resp));
            break;

        case 'NotFound':
            resp.type = 'NotFound';
            return this._cb ? this._cb(resp) : this._next(this._http.NotFound());
            break;
    ...
    return this._cb ? this._cb(resp) : this._next(this._http.InternalServerError(resp));
};

/**
 * ROUTES
 */
Schema.prototype.get = function (obj, cb) {
    if(cb) this._cb = cb;
    var self = this;
    var Item = self._model;

    Item.q(obj, this._config.query, function(err, doc) {
        if(err)
