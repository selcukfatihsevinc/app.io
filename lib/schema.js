        ...
        objectid: function (schema, candidate, cb) {
            ...
            */
            ...
            if(schema.belongs_to) {
                var props     = this._schema.properties;
                var belongsTo = _.where(props, {alias: schema.belongs_to.self});
                var origin    = this.origin;
                var alias     = m.schema.inspector.Alias;
            ...
            m.count(cond, function(err, count) {
...
Schema.prototype.setTz = function(timezone) {
...
Schema.prototype.dateFormat = function() {
    this._dateFormat = true;
    return this;
};

Schema.prototype.protect = function(type, owner) {
    var _owner = owner || this._owner; 
...
Schema.prototype.header = function(header, obj) {
    if(this._api)
...
Schema.prototype.getMaskFields = function (type, model) {
    var fields = false;
...
Schema.prototype.maskOne = function (type, doc, model) {
    var fields = this.getMaskFields(type, model);
...
Schema.prototype.getPromise = function (obj, self, cacheNotFound) {
...
Schema.prototype.populate = function (docs, opts, cb) {
	var self = this;
	var Item = self._model;
	opts = opts || {path: '_id'};
	
	Item.populate(docs, opts, function(err, populated) {
		if(self._format) {
			_.each(populated, function(doc) {
				if(doc && opts.path && doc[opts.path]) {
					var curr = JSON.parse(JSON.stringify(doc[opts.path]));
					self.from(curr);
					doc[opts.path] = curr;					
				}
			});
...
Schema.prototype.post = function (obj, cb) {
    ...
    obj = this.clone(obj);
    var log = true;
    var length = this._req && this._req.headers && this._req.headers['content-length'];
    if(length) {
        length = length/1024;
        if(length > 100) {
            log = false;
    ...
    if(log) this.log('POST:DATA:NAKED', obj);
    ...
    if(log) this.log('POST:DATA:MASKED', obj);
    ...
    this.to(obj);
    if(log) this.log('POST:DATA:CONVERTED', obj);
...
Schema.prototype.dismissHook = function(status) {
...
Schema.prototype.put = function (id, obj, cb) {
    ...
    if( ! this._master && this.protect('put') ) {
        var ownerField = this._alias[this._owner.alias];
    ...
    a.push(function(parsed, unset, cb) {
        var _update = self.clone(self._update);
        var _attrs  = ['pattern'];
        ...
        if(parsed.$set) {
            _.each(parsed.$set, function(value, key) {
                var props = self._save.properties[key];
        ...
        self.validate(_update, parsed, function(err, result) {
            if(result.error.length)
...
Schema.prototype.remove = function (id, cb) {
    ...
    if( ! this._master && this.protect('remove') ) {
        var alias = dot.get(this._owner, 'protect.remove.alias') || this._owner.alias;
        var field = this._alias[alias];
...
Schema.prototype.to = function (obj, keepEmpty) {
    ...
    _.each(obj, function(value, key) {
        ...
        var realKey = self._alias[key] || key; // alias'ın gerçek key karşılığını alıyoruz (field_one => pf.fo gibi)
        ...
        if(self._dateFormat && props) {
            var dateFormat = dot.get(props, 'settings.dateFormat');
            var ftype = dot.get(props, 'settings.field') || props.ftype;
...
Schema.prototype.from = function (doc, name, parent, parentDoc, parentAlias, parentKey) {
    var self     = this;
    var model    = name ? this.getModel(name) : this._model;
    name         = name || this._name;
    var save     = model.schema.inspector.Save;
    var keyAlias = model.schema.inspector.KeyAlias;
    ...
    _.each(doc, function(value, key) {
        ...
        var dynamic = dot.get(model.schema, 'paths.'+org+'.options.refPath');
        ...
        if(refs) {
            ...
            _.each(value, function(aVal, aKey) {
                if(self.type(aVal) == '[object Object]') {
                     ...
                     */
                    aVal = _.mapObject(aVal, function(val, key) {
                        return (self.type(val) == '[object Object]') ? val.toString(): val;
                    });
