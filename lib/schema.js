...
Schema.prototype.mask = function (type, doc) {
    ...
    if(this._app.lib.utils && this._mask && this._mask[type]) {
        var mask   = self._mask[type];
        var fields = mask.master || mask.owner || mask.user || mask.guest;
...
Schema.prototype.maskOne = function (type, doc) {
    ...
    if(this._app.lib.utils && this._mask && this._mask[type]) {
        ...
        // change doc
        return self._app.lib.utils.helper.mask(doc, fields);
    }
...
Schema.prototype.get = function (obj, options, cb) {
    ...
    if(this._app.core.cache && cacheKey) {
        ...
        this._app.core.cache.cached(cacheKey, this.getPromise, params)
            .then(function(doc) {
                self._log.schema(self._group+':CACHE', cacheKey);
...
Schema.prototype.post = function (obj, cb) {
    ...
    this._app.lib.logger.schema(this._group+':POST:DATA:MASKED', obj);
                ...
                if(parsed) {
                    _.each(ops, function(value, key) {
                        if(parsed[value]) {
                            ...
                            self._log.schema(self._group+':PUT:DATA:MASKED:'+value, parsed[value], self._api);
                    ...
                    // pullAll işlemi yapmayacağız
                    if(parsed.$pullAll)
                        delete parsed.$pullAll;
                }
