...
/**
 ...
 qt => query type (find|one|count|findcount|distinct)
 f  => fields
 s  => sort
 sk => skip
 l  => limit
...
var parse = function(query, model, opts) {
    ...
    try {
        ...
        _.each(paths, function(path, key) {
            ...
            else if( _(path).safe('options.alias') ) {
...
var doQuery = function(query, model, opts, cb) {
    ...
    switch(q.qt) {
        ...
        case 'search':
            if(typeof model.search == 'function') {
                model.search({match_all: {}}, {hydrate: false}, function(err, doc) {
                    cb(err, doc, q);
                });
            }
            else
        ...
        case 'distinct':
            q.f ? model.distinct(q.f, q.q, function(err, doc) {
    ...
    if( ['find', 'one', 'findcount', 'randomfind', 'randomone'].indexOf(qt) >= 0 ) {
        if(q.s)  m.sort(q.s);
        if(q.sk) m.skip(q.sk);
        if(q.l)  m.limit(q.l);
        if(q.f)  m.select(q.f);
    ...
    if( ['find', 'one', 'randomfind', 'randomone'].indexOf(qt) >= 0 ) {
