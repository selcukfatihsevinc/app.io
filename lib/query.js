...
var parse = function(query, model, opts) {
    ...
    try {
        ...
        var param = function(key, val) {
	        ...
	        // parse aggregation
	        if(lcKey == 'aggregate') {
		        var agrVals = val.split(',');
		        _.each(agrVals, function(agrVal) {
		            var opt = getOperator(agrVal);
            ...
            var paramOpts = ['empty', '!empty', 'null', 'sum', 'avg'];
            ...
            else if(['sum', 'avg'].indexOf(operator) >= 0) {
	            var aoObj = {};
