...
Denormalize.prototype.process = function (doc, inspector, cb) {
    ...
    _.each(vals, function(value, key) {
        ...
        (function(a, model, doc, value, mongoose, denorm, alias) {
            ...
            var target = denorm[model].target;

            if( ! target )
                return console.log('denormalize.process inspector.Denorm[model].target not found');
            ...
            var fields = denorm[model].fields.split(',');
...
Denormalize.prototype.touch = function (data, inspector) {
    ...
    if(denorm[source]) {
        var fields  = denorm[source].fields.split(',');
        var target  = denorm[source].target;
        ...
        if(_doc) {
            // source model
            var sModel = self._mongoose.model(name);
